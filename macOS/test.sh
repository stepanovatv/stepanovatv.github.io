...
if [ -d "$FRAMEWORKS/Testing.framework" ]; then
  swift test --sdk "$SCHEDULE_SDK_PATH" --build-system native --disable-xctest --enable-swift-testing --disable-sandbox --scratch-path "$BUILD_ROOT" --cache-path "$BUILD_ROOT/cache" \
  ...
  swift test --sdk "$SCHEDULE_SDK_PATH" --build-system native --disable-xctest --enable-swift-testing --scratch-path "$BUILD_ROOT"
fi
